...
export default function UserDashboard() {
  ...
  return (
    <div className="space-y-8 animate-fade-in p-6">
      <div>
        <h1 className="text-2xl font-bold">Welcome, {currentUser.name?.split(' ')[0] ?? 'User'}</h1>
        <p className="text-muted-foreground text-sm">Here are your assigned projects and tasks</p>
      </div>

      {/* Projects grid */}
      {myProjects.length > 0 && (
        <div>
          <h2 className="text-base font-semibold mb-3 text-slate-200">My Projects</h2>
      ...
      {/* My Tasks Kanban */}
      <div>
        <h2 className="text-base font-semibold mb-4 text-slate-200">My Tasks</h2>
        <KanbanBoard userId={currentUser.id} readonly={false} />
      </div>
    </div>
  );
}
